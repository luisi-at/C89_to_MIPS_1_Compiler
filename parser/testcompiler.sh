...
for i in test/programs/*; do
    ...
    mips-linux-gnu-gcc -c -S -x c $i/in.code.c -o working/$i/mips_gnu.ref.s
    #now compile to mips binary:
    mips-linux-gnu-gcc -static working/$i/mips_gnu.ref.s -o working/$i/mips_gnu.ref
    # single hyphen gives stdin/stdout
    #compile now to an object file and then run in qemu and get result
    #output result to file
    qemu-mips working/$i/mips_gnu.result

    REF_RESULT=$?;
    echo "${REF_RESULT}" > working/$i/ref.result.txt
    ...
    GOT_OUTPUT=$(echo $(cat working/$b/c_compiled.output.txt | ${DOS2UNIX} ))
